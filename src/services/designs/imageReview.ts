...
export function reviewGeneratedImage(input: {
  ...
  // Starter 009: designs must request both graphic imagery and slogan typography in one piece
  const promptLower = input.prompt.toLowerCase()
  const asksGraphic =
    promptLower.includes('illustration') ||
    promptLower.includes('imagery') ||
    promptLower.includes('hero image') ||
    promptLower.includes('cartoon') ||
    promptLower.includes('subject')
  const asksText =
    promptLower.includes('slogan') ||
  ...
  if (!asksText && !input.hasCompositedTypography) {
